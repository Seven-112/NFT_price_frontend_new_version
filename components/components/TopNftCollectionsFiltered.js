...
const TopNftCollectionsFiltered = () => {
  ...
  useEffect(() => {
    ...
    const fetchEtherPrice = async () => {
      ...
      const dataChain = await Axios.get(
      ...
      setChainsFilterData(dataChain.data.data);
    };
    fetchEtherPrice();
    // setLoading(false)
  }, [dispatch]);
  ...
  const AddChainsFilterData_ = (index, item) => {
    ...
    let newArray = topCollections.filter(function (el) {
      return el.chain.identifier == item;
    });
    setTopCollections_(newArray);
  }

  const AllAddChainsFilterData_ = (item) => {
    setTopCollections_active(item);
    setTopCollections_(topCollections);
  }
                      ...
                      <div className={`filter-list-body`}>
                        {/* <div className="filtered_list">
                          {chainsFilterData_.map((item, index) => (
                            <div key={index} className="filtered" onClick={() => RemoveChainsFilterData_(index)}>{item}&nbsp;<CancelIcon fontSize="small" /></div>
                          ))}
                        </div> */}
                        ...
                        <div className="" style={{ position: 'relative' }}>
                          ...
                          {topCollections_active === "All" ?
                            <div className="" style={{ color: 'green', position: 'absolute', right: '15px', top: '5px' }}>✔</div>
                            : <></>
                          }
                        </div>
            ...
            <div className="col-12">
              <div>
                {/* {loading || !etherPrice ? (
                  ...
                  <TopNftCollectionTable topCollections={isTopCollections_ ? topCollections_ : topCollections} time={time} payUnite={payUnite} etherPrice={etherPrice} />
                )} */}
                <TopNftCollectionTable setLoading={setLoading} topCollections={isTopCollections_ ? topCollections_ : topCollections} time={time} payUnite={payUnite} etherPrice={etherPrice} />
              </div>
            </div>
          </div>
        </section>
