...
export default function EnhancedTable(props) {
    const { topCollections, time, payUnite, etherPrice, setLoading } = props;
    var tokens = [];
    ...
    topCollections.map((topCollection, index) => {
        ...
        token.suplyRatio_ = topCollection.data.stats.supply_ratio_ratio;
        if (payUnite) {
            token.priceFloor = (topCollection.data.stats.floor_price) ? topCollection.data.stats.floor_price : 0;
            token.priceFloor_ = (topCollection.data.stats.floor_price) ? numFormatter(topCollection.data.stats.floor_price) : "N/A";
        } else {
            token.priceFloor = (topCollection.data.stats.floor_price) ? (topCollection.data.stats.floor_price) * etherPrice : 0;
            token.priceFloor_ = (topCollection.data.stats.floor_price) ? numFormatter((topCollection.data.stats.floor_price) * etherPrice) : "N/A";
        }
        ...
        tokens.push(token);
        if (index == topCollections.length - 1) {
            setLoading(false)
        }
                                        ...
                                        >
                                            ...
                                            <TableCell align="left" className='d-flex align-items-center '>
                                                ...
                                                <div style={{
                                                    ...
                                                    <a href={`/collections/${token.slug}`}>{token.collectible}</a>
                                                </div>

                                            </TableCell>
                                            <TableCell align="left" style={{ 'padding': "10px 10px 10px 10px" }} className='' >
                                                <i className={(payUnite) ? "fab fa-ethereum" : "fas fa-dollar-sign"}></i> {token.priceFloor_}
                                            </TableCell>
                                            ...
                                            <TableCell align="left" style={{ 'padding': "10px 0 10px 28px" }} className=''>
                                                <i className={(payUnite) ? "fab fa-ethereum" : "fas fa-dollar-sign"}></i>{token.volume_}
                                            </TableCell>
                                            <TableCell align="left" style={{ 'padding': "10px 0 10px 28px" }} className=''>
                                                {token.sales}
                                            </TableCell>
                                            <TableCell align="left" style={{ 'padding': "10px 0 10px 28px" }} className=''>
                                                <span> <p>{token.suplyRatio_}</p><p>{token.suplyRatio}</p>  </span>
                                            </TableCell>
                                            <TableCell align="left" style={{ 'padding': "10px 0 10px 28px" }} className=''>
                                                <i className={(payUnite) ? "fab fa-ethereum" : "fas fa-dollar-sign"}></i>{token.marketCap_}
                                            </TableCell>
                                        </TableRow>
                                    );
                                })}
